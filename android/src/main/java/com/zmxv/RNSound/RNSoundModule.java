...
public class RNSoundModule extends ReactContextBaseJavaModule implements Player.EventListener { //implements AudioManager.OnAudioFocusChangeListener {
  ...
  @ReactMethod
  public void prepare(final String fileName, final Double key, final ReadableMap options, Callback callback) {
    mCallback = callback;
    player = ExoPlayerFactory.newSimpleInstance(this.context);
    int res = this.context.getResources().getIdentifier(fileName, "raw", this.context.getPackageName());

    DataSpec dataSpec = new DataSpec(RawResourceDataSource.buildRawResourceUri(res));
  ...
  public void onPlayerStateChanged(final boolean playWhenReady, final int playbackState) {
    if (playbackState == Player.STATE_READY && mCallback != null) {
      WritableMap props = Arguments.createMap();
      mCallback.invoke(null, props);
      mCallback = null;
    }
  }
  ...
  public void setVolume(final Double key, final Float left, final Float right) {
    if (player != null) {
      Log.d("RN", "Setting volume to " + left);
